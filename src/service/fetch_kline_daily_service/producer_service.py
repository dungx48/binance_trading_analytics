...
class BinanceKlinesProducerService:
    def __init__(self):
        ...
        self.symbols = coin_service.get_all_coins()
    ...
    def get_klines_data(self, req: KlinesRequest) -> List:
        ...
        while current_time < req.end_time:
            ...
            params = {
                "symbol": f"{req.base_symbol}{req.quote_symbol}".upper(),
    ...
    def run_daily_producer(self, mode="daily"):
        # symbols = {coin + "USDT" for coin in coin_service.get_all_coins()}
        for symbol in tqdm(self.symbols, desc="🚀 Fetching Klines"):
            self.fetch_and_push(symbol, mode=mode)
            logging.info("="*80)
